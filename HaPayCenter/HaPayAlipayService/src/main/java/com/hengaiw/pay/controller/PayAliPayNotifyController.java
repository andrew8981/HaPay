...
public class PayAliPayNotifyController extends PayNotifyBase {
	...
	public String doAliPayRes(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 支付状态成功或者完成
		if (trade_status.equals(PayConstants.AlipayConstant.TRADE_STATUS_SUCCESS) ||
				trade_status.equals(PayConstants.AlipayConstant.TRADE_STATUS_FINISHED)) {
			...
			_log.info("{}响应给支付宝结果：{}", logPrefix, PayConstants.RETURN_ALIPAY_VALUE_SUCCESS);
			return PayConstants.RETURN_ALIPAY_VALUE_SUCCESS;
		}
		doNotify(payOrder);
