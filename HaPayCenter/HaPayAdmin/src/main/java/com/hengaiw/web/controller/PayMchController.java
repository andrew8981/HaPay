...
public class PayMchController {
	private final static HaLog _log = HaLog.getLog(PayMchController.class);
	@Value("${mch.reqKey}")
	 ...
	 * 查询商户信息
	 */
	@RequestMapping("/getMchInfo.html")
	public String getMchInfo(HttpServletRequest request) {
		...
		if(!StringUtils.isBlank(mchId)) {
			...
			_log.info("请求的参数:{}", reqData);
			result = HaPayUtil.call4Post(selectChannel_url + reqData);
			Map retMap = JSON.parseObject(result);
	        if("SUCCESS".equals(retMap.get("retCode"))) {
	            // 验签
	            String checkSign = PayDigestUtil.getSign(retMap, resKey, "sign");
	            String retSign = (String) retMap.get("sign");
	            _log.info("返回的结果:{}", JSON.toJSON(retMap));
